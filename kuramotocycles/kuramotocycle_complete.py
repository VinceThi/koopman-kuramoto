...
diff_sigma_12_k = np.cos((thetas[:, 1] + thetas[:, 2])/2 - thetas[:, 0]) + np.cos((thetas[:, 1] + thetas[:, 3] - delta_23)/2) \
                  + np.cos((thetas[:, 4] + thetas[:, 2] - delta_41)/2)
diff_sigma_23_k = [0 for _ in range(n_iter)]
diff_sigma_34_k = [0 for _ in range(n_iter)]
diff_sigma_41_k = [0 for _ in range(n_iter)]
