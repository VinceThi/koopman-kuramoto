...
def extract_invariants(graph, n, maps, number_motifs, stars=False):
    invariants = []
    for j in range(len(n)):
        for i in range(n[j]):
            # Get vertices in motif
            list_vertices = list(maps[j][i].get_array())
            # if the motifs are 5-stars, exclude the center
            if stars:
...
def eliminate_duplicates(constants):
    constants_no_duplicates = set(tuple(constant) for constant in constants)
    return [list(constant) for constant in constants_no_duplicates]
