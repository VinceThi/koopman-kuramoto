...
def objective_function_init_cond_ORIGINAL_VERSION(x, theta0):
...
def get_watanabe_strogatz_initial_conditions_ORIGINAL_VERSION(theta0, dispersed_guess=False, nb_guess=5000, tol=1e-10):
    ...
    for _ in tqdm(range(nb_guess)):
        ...
        solution = root(objective_function_init_cond_ORIGINAL_VERSION, initial_guess, jac=jacobian_matrix_objective_function,
...
def objective_function_init_cond_ARCTAN(x, theta0): # NOT WORKING
    """ R, Theta, Phi, psi_1, ..., psi_N = x
     where Z = R e^(i Theta), w_j = e^(i psi_j), Phi = Theta - phi """
    return np.concatenate([((theta0 - x[1])/2 + np.pi) % (2*np.pi) - np.pi - np.arctan((1 - x[0])/(1 + x[0])*np.tan((x[3:] - x[2])/2)),
                           np.array([np.sum(np.sin(x[3:])), np.sum(np.cos(x[3:])), x[3]])])


def get_watanabe_strogatz_initial_conditions_ARCTAN(theta0, dispersed_guess=False, nb_guess=5000, tol=1e-10): # NOT WORKING
    """ Warning: Choosing a too low tolerance can cause problems, the unit test
     'test_get_watanabe_strogatz_initial_conditions' is not successful for tol=1e-8. tol=1e-10 seems to be sufficient.
     Note that theta0 must not be a state of majority cluster (see Watanabe-Strogatz, Sec. 4.2.3., 1994). """
    N = len(theta0)
    ...
    for _ in tqdm(range(nb_guess)):
        ...
        psi = 2*np.pi*np.random.random(N)
        initial_guess = np.concatenate([np.array([R, Theta, Phi]), psi])
        solution = root(objective_function_init_cond_ARCTAN, initial_guess,
                        args=(theta0,), method='hybr', options={'xtol': tol, 'col_deriv': True})
        if solution.success:
            if 0 < solution.x[0] < 1:
                break
