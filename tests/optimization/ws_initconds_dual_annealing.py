...
def get_watanabe_strogatz_initial_conditions(theta0, dispersed_guess=False, nb_guess=1, tol=1e-10):
    ...
    for _ in tqdm(range(nb_guess)):
        ...
        solution = dual_annealing(objective_function_init_cond, bounds, args=(theta0,), maxiter=10000, accept=1)

        # NOTE: this part should be superfluous since bounds are specified
        if solution.success:
            if 0 < solution.x[0] < 1:
                break
