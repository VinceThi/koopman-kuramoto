...
from dynamics.watanabe_strogatz import ws_equations_kuramoto, ws_transformation, inverse_ws_transform
...
# Z0, phi0, w = 0.5, 1, np.exp(1j*np.array([0.1, 0.5, 0.2, 1]))
...
vf = []
for i in range(len(timelist)):
    wt = inverse_ws_transform(Z0, phi0, np.exp(1j*theta[i, :]))
    angle = np.angle(ws_transformation(Z[i], phi[i], wt))
...
plt.subplot(211)
# plt.plot(d, color=deep[0], label="Derivative $\\theta$")
plt.plot(vf, color=deep[2], label="$F(\\theta)$")
# plt.plot(derivative_theta_transformed, color=deep[1], linestyle="--", label="Derivative $\\theta$ transformed")
plt.plot(vector_field, color=deep[4], linestyle="--", label="$F$(transformed $\\theta$)")
...
for i in range(len(theta[0, :])):
    if not i:
        plt.plot(timelist, theta[:, i], color=deep[0], label="Kuramoto")
        plt.plot(timelist, theta_ws[:, i], color=deep[1], linestyle="--", label="WS")
        plt.plot(timelist, theta_transformed[:, i], color=deep[2], linestyle="dotted", label="Transformed solution")
    else:
        plt.plot(timelist, theta[:, i], color=deep[0])
        plt.plot(timelist, theta_ws[:, i], color=deep[1], linestyle="--")
        plt.plot(timelist, theta_transformed[:, i], color=deep[2], linestyle="dotted")
