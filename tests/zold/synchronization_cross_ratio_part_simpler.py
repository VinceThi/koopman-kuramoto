...
@njit(fastmath=True)
def real_ws_equation(t, state, w, calA_source, calA_row_periphery, Omega):
    rho = state[0]
    psi = state[1]
    epsi = np.exp(1j*psi)
    epsibar = np.exp(-1j*psi)
    transfo = (w - rho*epsi)/(1 + rho*epsibar*w)
    interaction_term = rho*epsibar*(np.sum(calA_row_periphery*transfo) + calA_source)
    dotrho = np.real((1/rho - rho)*np.real(interaction_term))
    dotpsi = np.real((1 - 1/rho**2)*np.imag(interaction_term))
    return dotrho, dotpsi
...
if import_setup:
    ...
    for i, w1 in enumerate(w1_crossratio):
        ...
        solution = np.array(integrate_dopri45_jit(t0, t1, dt, ws_equations_kooku1_fig3,
                            np.array([Z0, phi0], dtype=complex), *args_ws))
        ...
        """ Integrate the cross-ratio part from real equations """
        zeta0 = Z0*np.exp(-1j*phi0)
        rho0, psi0 = np.abs(zeta0), np.angle(zeta0)
        print(rho0, psi0)
        solution2 = np.array(integrate_dopri45_jit(t0, t1, dt, real_ws_equation,[rho0, phi0], *args_ws))
        rho = solution2[:, 0]
        psi = solution2[:, 1]
        zeta_fromreal = rho*np.exp(1j*psi)
        ...
        if plot_trajectories:
            ...
            ax1.plot(np.real(zeta_fromreal), np.imag(zeta_fromreal), color="#aac4ff", label=r"From real eqs")
            ax1.scatter(np.real(zeta[0]), np.imag(zeta[0]), color="#aac4ff")
            ax1.axis('off')
            ax1.legend(loc=1)
            
            ax2.plot(timelist, np.abs(zeta))
            ax2.plot(timelist, rho)
