...
# np.random.seed(2333)
# np.array([4.31159804, 3.52251869, 0.584871, 2.27442686])  #
...
for i in range(len(timelist)):
    ...
    vector_field_hattheta.append(np.real(-1j*np.conj(hattheta[i, :]) *
                                         ricatti(0, hattheta[i, :], theta, i, omega, coupling)))
    # vector_field_hattheta1.append(kuramoto_sakaguchi(0, hattheta1[i, :], W, coupling, omega, alpha))
    vector_field_hattheta_expected.append(np.real(-1j*np.conj(transformed_ricatti[i, :]) *
                                                  ricatti(0, transformed_ricatti[i, :], theta, i, omega, coupling)))
...
for i in range(len(theta[0, :])):
    if i == 0:
        plt.plot(timelist, theta[:, i], color=reduced_grey, label="Kuramoto solution $\\theta(t)$")
        plt.plot(timelist, theta_ricatti[:, i], color=complete_grey, linestyle=(0, (5, 10)),
                 label="Equivalent Ricatti solution")
    else:
        plt.plot(timelist, theta[:, i], color=reduced_grey)
        plt.plot(timelist, theta_ricatti[:, i], color=complete_grey, linestyle=(0, (5, 10)))
plt.ylabel("Phase") 
plt.xlabel("Time $t$")
...
for i in range(len(theta[0, :])):
    if i == 0:
        ...
        plt.plot(timelist, hattheta[:, i], color=complete_grey, linewidth=2,
                 linestyle=(0, (5, 10)), label="Transformed $\\hat{\\theta}(t)$")
        plt.plot(timelist, hattheta_kur[:, i], color=reduced_grey,
                 linestyle="--", label="Kuramoto for these init. cond.")
        ...
        plt.plot(timelist, hattheta[:, i], color=complete_grey, linestyle=(0, (5, 10)))
        plt.plot(timelist, hattheta_kur[:, i], color=reduced_grey)
...
for i in range(len(theta[0, :])):
    if i == 0:
        plt.plot(vector_field_hattheta_expected[:, i], color=deep[2], label="Expected $F(\\hat{\\theta}(t)$)")
        ...
        plt.plot(vector_field_hattheta[:, i], color=complete_grey, linestyle="--", label="$F(\\hat{\\theta}(t))$")
        ...
        plt.plot(vector_field_hattheta_expected[:, i], color=deep[2])
        ...
        plt.plot(vector_field_hattheta[:, i], color=complete_grey, linestyle="--")
