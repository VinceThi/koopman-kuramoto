...
if import_setup:
    ...
    file_path = Path(path / "2025_08_13_23h44min39sec_test_kuramoto_parameters_dictionary.json")
    load_parameters_to_globals(file_path, keys_as_array=[
        "sizes_monomial", "sizes_crossratio", "size_nonintegrable",
        "W", "alpha", "omega", "C", "chi", "theta0", "theta", "random_exponents",
    ...
    meanW_crossratio = -0.5
    stdW_crossratio = 0
    weights_crossratio = np.random.normal(meanW_crossratio, stdW_crossratio, (len(sizes_crossratio), N))
    weights_crossratio[1, 0] = 1
    weights_crossratio[1, 1] = 1
    weights_crossratio[1, 2] = 1
    ...
    phaselags_monomial = np.random.normal(meanalpha_monomial, stdalpha_monomial, (sum(sizes_monomial), sum(sizes_monomial))) % (np.pi/2)
    phaselags_crossratio = np.random.normal(meanalpha_crossratio, stdalpha_crossratio, (len(sizes_crossratio), N))
    phaselags_crossratio[1, 0] = 0
    phaselags_crossratio[1, 1] = 0
    phaselags_crossratio[1, 2] = 0
...
if plot_trajectories:
    eiphi = np.exp(1j * phi)[:, None]  # (T,1)
    Zc = np.conjugate(Z)[:, None]  # (T,1)
    num = eiphi * w[None, :] + Z[:, None]  # (T,N)
    den = 1.0 + eiphi * Zc * w[None, :]  # (T,N)
    theta_ws = np.angle(num / den)  # (T,N)
    theta_ws = np.where(theta_ws < 0, 2 * np.pi + theta_ws, theta_ws)
    # print(np.abs((theta_ws % (2*np.pi)) - (theta[:, 7:] % (2*np.pi))))
    mean_error_true_vs_ws = np.mean(np.abs((theta_ws % (2 * np.pi)) - (theta[:, 7:] % (2 * np.pi))))
    # plt.plot(mean_error_true_vs_ws)
    # plt.show()
    assert mean_error_true_vs_ws < 1e-2
...
# Plot trajectory of Z exp(i phi)
ax1.plot(np.real(zeta), np.imag(zeta), color="#aac4ff", label=r"$Z(t) e^{-i\phi(t)}$")
ax1.scatter(np.real(zeta[0]), np.imag(zeta[0]), color="#aac4ff", label="start")
# ax.axhline(0, color=complete_grey, lw=0.5)
# ax.axvline(0, color=complete_grey, lw=0.5)
# ax.set_xlabel("Re")
# ax.set_ylabel("Im")
ax1.axis('off')
ax1.legend(loc=1)
...
if not import_setup or simulate:
    ...
    if reply == QMessageBox.StandardButton.Yes:
        ...
        parameters_dictionary = {"N": N, "sizes_monomial": sizes_monomial.tolist(),
                                 ...
                                 "Rew": Rew.tolist(), "Imw": Rew.tolist(),
                                 "timelist": timelist.tolist(), "ReZ": ReZ.tolist(),
                                 "ImZ": ImZ.tolist(), "phi": phi.tolist(),
