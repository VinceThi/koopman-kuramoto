...
phi0 = 2  # 2*np.arcsin(Y0/np.sqrt(1 + R0**2))
Psi0 = 3    # Phi0 + phi0/2
...
for i in range(len(theta[0, :])):
    if i == 0:
        plt.plot(timelist, hattheta_b[:, i], color=deep[1], zorder=10,
        ...
        plt.plot(timelist, hattheta_b[:, i], color=deep[1], linestyle="--", zorder=10)
        plt.plot(timelist, hattheta_expected_b[:, i], color=total_color, linestyle="-")
plt.plot(timelist, Psi, label="$\\Psi$", linewidth=4)
plt.ylabel("Phase")
plt.xlabel("Time $t$")
