...
def coefficient_matrix_size_2_deg_1(truncation_dimension):
    ...
    d1 = np.array([1] + (trd - 2) * [0])
    d2 = 1/2*(2*np.arange(1, trd-1, 1)+3+(-1)**(np.arange(1, trd-1, 1) + 1))
    dm2 = 1/2*(-2*np.arange(1, trd-1, 1)+1+(-1)**(np.arange(1, trd-1, 1)))
    return np.diag(d1, k=1) + np.diag(d1, k=-1) \
        + np.diag(d2, k=2) + np.diag(dm2, k=-2)
...
for td in [10, 15, 20, 25]:  # np.arange(20, 101, 21):
...
vaps = np.imag(eigvals(A))
weights = np.ones_like(vaps) / float(len(vaps))
plt.hist(vaps/np.max(np.abs(vaps)), bins=100,
...
#     rescaled_eigenvalues = sorted_imag_part_eig / td
#     # rescaled_indices = np.arange(1, len(rescaled_eigenvalues) + 1)/td
#     rescaled_indices = np.linspace(-td // 2, td // 2,
#                                    len(rescaled_eigenvalues)) / td
#     plt.scatter(rescaled_indices, rescaled_eigenvalues,
#                 label=f"$d = {td}$",
#                 s=s)  # , color=deep[i])
#     if td == max(d_array):
