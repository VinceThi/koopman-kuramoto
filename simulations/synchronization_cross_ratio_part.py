...
@njit(fastmath=True)
def ws_equations_kooku1_fig3(t, state, theta_series, current_index,      # Not used anymore: theta_series, current_index, but useful if other sources are added, see synchronization_cross_ratio_part_slow()
                             w, calA_source, calA_row_periphery, Omega):
    """
    Hardcoded for Fig. 3 (a). This is the equation in the frame of the source
      state = [Z, phi] (complex128)
      theta_series[i] -> theta at timestep i (float64 vector of size N)
      theta_series is assumed to be of shape (n_steps, N); we use current_index 'i' to take the row.
...
if import_setup:
    ...
    sizes_crossratio = np.array([93], dtype=int)
    ...
    probabilities_crossratio = np.array([[1, 0.8]])
