...
probabilities_crossratio = np.array([[1, 0.9, 0],  
                                     [1, 0., 0.9]])
# np.random.rand(c, N)
...
for t in tqdm(range(len(timelist))):
    ...
    args_ws1 = (z1_t, cal_A[0, 0], cal_A[0, 1:1+sizes_crossratio[0]], Omega1, zs_t)
    ...
    args_ws2 = (z2_t, cal_A[1, 0], cal_A[1, 1+sizes_crossratio[0]:], Omega2, zs_t)
