...
def integrate_rk4(t0, t1, dt, dynamics, init_cond, *args):
    f = dynamics
...
def integrate_dopri45(t0, t1, dt, dynamics, init_cond, *args):
    f = dynamics
    tvec = np.arange(t0, t1, dt)
    sol = [init_cond]
    for i, t in enumerate(tvec[0:-1]):
        ...
        k4 = f(t + 4./5*dt, sol[i] + dt*(44./45*k1 - 56./15*k2 + 32./9*k3), *args)
        k5 = f(t + 8./9*dt, sol[i] + dt*(19372./6561*k1 - 25360./2187*k2 + 64448./6561*k3 - 212./729*k4), *args)
        k6 = f(t + dt, sol[i] + dt*(9017./3168*k1 - 355./33*k2 + 46732./5247*k3 + 49./176*k4 - 5103./18656*k5), *args)
...
def integrate_dynamics(t0, t1, dt, dynamics, adjacency_matrix, integrator, init_cond, *args, print_process_time=False):
