...
def torus_coordinates(theta1, theta2, R=2, r=1):
...
# Plot the torus surface with shading
...
shading = ls.shade(Z_torus, cmap=plt.cm.gray, vert_exag=0.1, blend_mode='soft')
ax.plot_surface(X_torus, Y_torus, Z_torus, facecolors=shading, rstride=1, cstride=1, linewidth=0, antialiased=True, edgecolors='none')

# Plot the vector field on a torus with adjusted zorder
# ax.quiver(X, Y, Z, U, V, W, length=0.15, color='black', alpha=1)
...
# ax.quiver(X, Y, Z, U, V, W, length=0.5, color='#2b2b2b')
#
# # Plot the trajectory on the torus
# ax.plot(traj_X, traj_Y, traj_Z, color='#404040', linewidth=2)
#
